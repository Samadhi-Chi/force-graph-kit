...
private func close(_ a: Double, _ b: Double, tolerance: Double = tolerance) -> Bool {
  abs(a - b) <= tolerance
}
...
@Test func manyBodySignsAndDistanceBounds() {
  func velocity(strength: Double, minimum: Double = 0, maximum: Double = .infinity) -> Double {
    var simulation = ForceSimulation(
      nodes: [ForceNode(id: 0, x: 0), ForceNode(id: 1, x: 0.5)], dimensions: .one)
    simulation.velocityDecay = 0
    simulation.alphaDecay = 0
    simulation.force(
      "charge", .manyBody(strength: strength, minimumDistance: minimum, maximumDistance: maximum))
    simulation.tick()
...
@Test func invalidParametersAreNoOpsAndRemainFinite() {
  var simulation = ForceSimulation(
    nodes: [ForceNode(id: 0, x: 0), ForceNode(id: 1, x: 1)], dimensions: .three)
...
@Test func snapshotsAndDragReheatFlow() {
  var simulation = ForceSimulation(
    nodes: [ForceNode(id: "drag", x: 1, y: 2, vx: 3)], dimensions: .two)
  simulation.updateNode(id: "drag") {
    $0.fx = 9
    $0.fy = 8
  }
