...
public struct ForceSimulation<ID: Hashable & Sendable>: Sendable {
    ...
    public init(nodes: [ForceNode<ID>] = [], dimensions: SimulationDimensions = .two, seed: UInt32 = 1) {
        self.nodes = nodes
    ...
    public mutating func tick(iterations: Int = 1) {
        for _ in 0..<max(0, iterations) {
            ...
            for index in nodes.indices { Self.integrate(&nodes[index], dimensions: dimensions, retention: retention) }
    ...
    public func snapshots() -> [NodeSnapshot<ID>] {
        nodes.map {
            NodeSnapshot(id: $0.id, x: $0.x, y: $0.y, z: $0.z,
    ...
    public func find(x: Double, y: Double = 0, z: Double = 0, radius: Double? = nil) -> ForceNode<ID>? {
        ...
        if let radius, (!radius.isFinite || radius < 0) { return nil }
        var best: ForceNode<ID>?
        var bestSquared = radius.map { $0 * $0 } ?? .infinity
        for node in nodes {
            let dx = node.x - x
            let dy = dimensions.rawValue > 1 ? node.y - y : 0
            let dz = dimensions.rawValue > 2 ? node.z - z : 0
            let squared = dx * dx + dy * dy + dz * dz
            if squared < bestSquared || (squared == bestSquared && best == nil) {
                bestSquared = squared
                best = node
            }
        }
    ...
    private static func integrate(
        ...
        if dimensions.rawValue > 1 { integrateAxis(position: &node.y, velocity: &node.vy, fixed: node.fy) }
        if dimensions.rawValue > 2 { integrateAxis(position: &node.z, velocity: &node.vz, fixed: node.fz) }

        func integrateAxis(position: inout Double, velocity: inout Double, fixed: Double?) {
            if let fixed, fixed.isFinite { position = fixed; velocity = 0 }
            else { velocity *= retention; position += velocity }
            if !position.isFinite { position = 0; velocity = 0 }
        }
    }
}
